    ...
    public class atop61850Client : atop61850Base
    {
        ...
        public void Poll_Static()
        {
            foreach (var item in atopDataMapping.VisualMapping)
            {
                ...
                {
                    if (Carried_Data.Trim() == string.Empty || arrBackend_Data[1].Trim() == string.Empty)
                        continue;

                    if (Convert.ToInt16(Carried_Data.Trim()) != Convert.ToInt16(arrBackend_Data[1].Trim()))
                    {
                        /* 測試失敗 */
                        atopLog.WriteLog(atopLogMode.TestFail, $"{item.IEC61850_TAGNAME} Value: {Carried_Data.Trim()} , Backend Type : {item.FUNCTION} Addr : {item.ADDRESS} Value : {arrBackend_Data[1].Trim()}");
                        continue;
                    }
                }
            }
